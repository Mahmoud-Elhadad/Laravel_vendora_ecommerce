<div class="swiper-slide">
    <article class="product-card fade-in-up" data-id="{{ $product->id }}" data-stock="{{ $product->count }}" data-discount="{{ $product->discount }}">
        <div class="pc-media">
            ...
            <div class="pc-badges">
                <span class="badge-v badge-new">New</span>
            </div>
        ...
        <div class="pc-body">
            ...
            <div class="pc-price">
                <span class="now">${{ $product->price - ($product->price * $product->discount / 100) }}</span><span class="old">${{ $product->price }}</span><span class="off">-{{ $product->discount }}%</span>
            </div>
            @if(Auth::guard("ecomm")->check())
